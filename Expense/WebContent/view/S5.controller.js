...
var oToken;
sap.ui.core.mvc.Controller.extend("Expense.view.S5", {
     companydescription:"",
     plantdescription:"",
     taxdescription:"",
     vendvalue1:"",
     ...
     onInit: function() {
           ...
           onSelectone:function(evt){
                this.mprest="";
                this.oModel = this.getOwnerComponent().getModel();
                this.getView().byId("itab208").destroyContent();
                this.getView().byId("itab209").destroyContent();
                this.getView().byId("itab3").destroyContent();
           ...
           {
                if(oEvent.mParameters.name=="electricity"){
                     this.mprest="";
                     var fragment1= new sap.ui.xmlfragment("Expense.view.abc",this);
                     this.getView().byId("itab208").addContent(fragment1);
                     var oFileUpload = sap.ui.getCore().byId('fileupload2');
                     oFileUpload.setUseMultipart(false);
                     var mockData = {
                                dataitems : [
                     ...
                     var data = {};
                      var oController = this;
                     this.oModel.read("/PROPOSALDETAILSSet(IvContype=%27208%27)",null,false,false,function(oData,response){
                     ...
                     this.oModel.read("/GETVENDORJCSet(Contype='208',OtherInd='',ImprestInd='I')",null,false,false,function(oData,response){
                           
                           data=oData;
                           this.vendvalue1=data.EvVendor;
                          sap.ui.getCore().byId("vend2").setValue(this.vendvalue1);
                          sap.ui.getCore().byId("pabl").setValue(this.vendvalue1.substring(vendvalue1.indexOf("-")+1,vendvalue1.length));
                           sap.ui.getCore().byId("pabl").setEditable(false);
                           var vfvendor=sap.ui.getCore().byId("vend2").getValue();
                           if(this.mprest==="" && (vfvendor==="" || vfvendor===undefined)){
                                sap.m.MessageBox.show("Please maintain imprest vendor in the user table !! Proposal can not be created");
                          ...
                          // var pryear = x.d.Mjahr;
                           sap.m.MessageBox.show(msg,{
                                ...
                                onClose : function(){
                                     ...
                                     that.ccode="";
                                     that.recode="";
                                     that.getView().byId("itab209").destroyContent();
                                     that.getView().byId("itab208").destroyContent();
                                     that.getView().byId("itab3").destroyContent();
                ...
                fileUpload:function(){
                     ...
                     var mockDataModel = new sap.ui.model.json.JSONModel(mockData);
                     oFileUpload.setModel(mockDataModel);
                     delete this.oModel.mCustomHeaders["X-CSRF-Token"];
                },
                ...
                onDeleteFile : function(oEventData) {
                     ...
                     fileupload.setCustomHeader('slug',exte+"|X");
