...
var oToken;
sap.ui.core.mvc.Controller.extend("Expense.view.S6", {
  companydescription:"",
  plantdescription:"",
  taxdescription:"",
  vendvalue1:"",
...
onSelectone:function(evt){
  this.mprest="";
  this.oModel = this.getOwnerComponent().getModel();
this.getView().byId("itab019").destroyContent();
  this.getView().byId("itab022").destroyContent();
  this.getView().byId("itab006").destroyContent();
  this.getView().byId("itab018").destroyContent();
...
{
  if(oEvent.mParameters.name=="employee"){
    this.mprest="";
    var fragment1= new sap.ui.xmlfragment("Expense.view.abc",this);
    this.getView().byId("itab019").addContent(fragment1);
    var oFileUpload = sap.ui.getCore().byId('fileupload2');
        oFileUpload.setUseMultipart(false);
        var mockData = {
         dataitems : [
  ...
  sap.ui.getCore().setModel(this.oModel);
     ...
     var data = {};
     var oController=this;
     this.oModel.read("/PROPOSALDETAILSSet(IvContype=%27019%27)",null,false,false,function(oData,response){
     ...
     this.oModel.read("/GETVENDORJCSet(Contype='019',OtherInd='',ImprestInd='I')",null,false,false,function(oData,response){
   ...
   onBack:function(){
     ...
     sap.ui.getCore().byId("ven").setVisible(false);
      ...
      // sap.ui.getCore().byId("wr").setVisible(true);
       // sap.ui.getCore().byId("vend2").setVisible(true);
     ...
     that.oModel.create("/ProposalCreateSet",saveData,null,function(data, request) {
             ...
             sap.m.MessageBox.show(msg,{
                   ...
                   onClose : function(){
                     ...
                     sap.ui.getCore().byId("itr2").setState(false);
                     that.ccode="";
                     that.getView().byId("itab019").destroyContent();
                  that.getView().byId("itab022").destroyContent();
                  that.getView().byId("itab006").destroyContent();
                  that.getView().byId("itab018").destroyContent();
